...
function Navbar() {
  const [True, setBoolean]=useState(true);
  return (
    <div 
    className='nav-fst-div fixed z-[999] w-full px-20 py-5 flex justify-between backdrop-blur-sm overflow-hidden' 
    style={{fontFamily:'NM'}}>
      <div className="logo">
        <svg
          width="72"
      ...
      <div className="links flex gap-4 mt-1">
      
        {["Services", "Our Work", "About us", "Insights","Contact"].map((item, index) => {
          return <a className={`text-md font-light ${index===4 && "lg:ml-[10vw]"}`} key={index}>{item}</a>;
        })}
      </div>
    </div>
  );
